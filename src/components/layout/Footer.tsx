import { MapPin, Phone, Mail, Globe, Facebook, Twitter, Instagram, Youtube } from 'lucide-react';

const Footer = () => {
  const quickLinks = [
    { label: 'Tentang Kami', href: '#' },
    { label: 'Program Studi', href: '#' },
    { label: 'Pendaftaran', href: '#' },
    { label: 'Beasiswa', href: '#' },
    { label: 'Karir', href: '#' },
  ];

  const academicLinks = [
    { label: 'Fakultas & Prodi', href: '#' },
    { label: 'Kalender Akademik', href: '#' },
    { label: 'E-Learning', href: '#' },
    { label: 'Perpustakaan', href: '#' },
    { label: 'Jurnal Ilmiah', href: '#' },
  ];

  const socialLinks = [
    { icon: Facebook, href: '#', label: 'Facebook' },
    { icon: Twitter, href: '#', label: 'Twitter' },
    { icon: Instagram, href: '#', label: 'Instagram' },
    { icon: Youtube, href: '#', label: 'YouTube' },
  ];

  return (
    <footer className="bg-footer text-footer-foreground">
      <div className="section-container py-12 lg:py-16">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 lg:gap-12">
          {/* About */}
          <div>
            <div className="flex items-center gap-2 mb-4">
              <div className="w-10 h-10 bg-primary rounded-full flex items-center justify-center">
                <span className="text-primary-foreground font-heading font-bold text-lg">U</span>
              </div>
              <div>
                <div className="font-heading font-bold text-lg">AMIKOM</div>
                <div className="text-xs text-footer-foreground/60">Yogyakarta</div>
              </div>
            </div>
            <p className="text-sm text-footer-foreground/80 leading-relaxed mb-4">
              Universitas AMIKOM Yogyakarta adalah perguruan tinggi swasta yang berfokus pada bidang teknologi informasi dan komunikasi di Indonesia.
            </p>
            <div className="flex gap-3">
              {socialLinks.map((social) => (
                <a
                  key={social.label}
                  href={social.href}
                  className="w-9 h-9 bg-footer-foreground/10 hover:bg-primary rounded-full flex items-center justify-center transition-colors"
                  aria-label={social.label}
                >
                  <social.icon className="w-4 h-4" />
                </a>
              ))}
            </div>
          </div>

          {/* Quick Links */}
          <div>
            <h4 className="font-heading font-semibold text-lg mb-4">Tautan Cepat</h4>
            <ul className="space-y-2">
              {quickLinks.map((link) => (
                <li key={link.label}>
                  <a href={link.href} className="footer-link text-sm">
                    {link.label}
                  </a>
                </li>
              ))}
            </ul>
          </div>

          {/* Academic */}
          <div>
            <h4 className="font-heading font-semibold text-lg mb-4">Akademik</h4>
            <ul className="space-y-2">
              {academicLinks.map((link) => (
                <li key={link.label}>
                  <a href={link.href} className="footer-link text-sm">
                    {link.label}
                  </a>
                </li>
              ))}
            </ul>
          </div>

          {/* Contact Info */}
          <div>
            <h4 className="font-heading font-semibold text-lg mb-4">Kontak</h4>
            <ul className="space-y-3">
              <li className="flex items-start gap-3">
                <MapPin className="w-5 h-5 text-primary flex-shrink-0 mt-0.5" />
                <span className="text-sm text-footer-foreground/80">
                  Jl. Ring Road Utara, Condongcatur,<br />
                  Depok, Sleman, Yogyakarta 55283
                </span>
              </li>
              <li>
                <a href="tel:+62274884201" className="flex items-center gap-3 footer-link">
                  <Phone className="w-5 h-5 text-primary flex-shrink-0" />
                  <span className="text-sm">(0274) 884201 - 207</span>
                </a>
              </li>
              <li>
                <a href="mailto:amikom@amikom.ac.id" className="flex items-center gap-3 footer-link">
                  <Mail className="w-5 h-5 text-primary flex-shrink-0" />
                  <span className="text-sm">amikom@amikom.ac.id</span>
                </a>
              </li>
              <li>
                <a href="https://amikom.ac.id" className="flex items-center gap-3 footer-link">
                  <Globe className="w-5 h-5 text-primary flex-shrink-0" />
                  <span className="text-sm">www.amikom.ac.id</span>
                </a>
              </li>
            </ul>
          </div>
        </div>
      </div>

      {/* Copyright */}
      <div className="border-t border-footer-foreground/10">
        <div className="section-container py-6">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4 text-sm text-footer-foreground/60">
            <p>© 2024 Universitas AMIKOM Yogyakarta. All Rights Reserved.</p>
            <div className="flex gap-4">
              <a href="#" className="hover:text-footer-foreground transition-colors">Kebijakan Privasi</a>
              <a href="#" className="hover:text-footer-foreground transition-colors">Syarat & Ketentuan</a>
            </div>
          </div>
        </div>
      </div>
    </footer>
  );
};

export default Footer;
